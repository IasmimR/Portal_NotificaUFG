...
define.Class('wixapps.core.views.ViewBuilder', function (classDefinition) {
    ...
    def.methods({
        ...
        getListViewTemplateDefinition: function (viewName, baseView) {
            var listTemplateViewDef = this._getViewDef("Array", "PaginatedListTemplate");
        ...
        _fixFieldTemplateData: function (fieldTemplateDef, dataFieldName) {
            ...
            for (var i = 0; i < bound.length; i++) {
                var data = bound[i].data.split(".").filter(function (s) {
                    return s.length;
                });
                if (data.length === 0) {
                    bound[i].data = dataFieldName;
                }
                else if (data[0] === "this") {
                    data[0] = dataFieldName;
                    bound[i].data = data.join(".");
                }
                else {
